...
// Browsers may require a real user gesture before audio can play. Unlock the
// splash sting first; updateMusicPlayback() starts the music after it ends.
function primeMusic() {
  playSplashSound();
...
// The Android app talks to Google Play Billing directly behind
// window.AndroidBilling (see SubscriptionBridge.kt) — no RevenueCat layer for
// now, so there's no backend tying a purchase to our own player_id yet; it
// only tracks whatever the signed-in Play Store account bought. The web
// build still expects a RevenueCat project through the
// @revenuecat/purchases-js SDK loaded in index.html — that side is unrelated
// and untouched by the Android migration. Both paths converge on subState so
// the rest of the game never branches on platform.
...
// Product ID created in Google Play Console. RevenueCat must import this
// product and expose it in the current offering.
const PLAY_SUBSCRIPTION_PRODUCT_ID = 'suscripcion_id_001';
// Reference price configured in Play Console. Local prices are generated by
// Google Play per country/region; the client must not convert this manually.
const SUBSCRIPTION_BASE_PRICE_PEN = 7.90;
...
const Subscription = (() => {
  ...
  function applyStatus(active, expiresAt) {
    // expiresAt isn't available without a backend verifying the purchase
    // (see SubscriptionBridge.kt), so the monthly coin grant keys off the
    // current calendar month instead of the real renewal date.
    const period = active ? new Date().toISOString().slice(0, 7) : null;
  ...
  async function purchase() {
    ...
    try {
      ...
      if (!pkg) { showSubscriptionError('No hay oferta de suscripcion configurada.'); return; }
      await rc().getSharedInstance().purchase({ rcPackage: pkg });
      await refreshStatus();
    } catch (e) { showSubscriptionError('Compra cancelada o fallida.'); console.warn(e); }
  }
...
window.onAndroidSubscriptionError = (message) => { console.warn('Suscripcion:', message); showSubscriptionError(message); };
...
let checkpointUsedThisLevel = false;
camX = 0;
deaths = 0;
totalDeaths = 0;
particles = [];
...
async function advanceAfterComplete() {
  ...
  advancingLevel = true;
  // Completing a level refunds exactly one flag when at least one was used.
  // Therefore one used flag is recovered, while two or more still cost one
  // net flag. Runs without a checkpoint do not change the balance.
